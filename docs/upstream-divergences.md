...
| `templates/match_play.html`, `static/js/match_play.js`, `web/match_play.go`, `field/arena.go` | Match Play carries a WPA key field per station alongside the team number, and Register stores the keys before substituting so the access point push picks them up. A blank key leaves the team's existing one. | Upstream sets keys from the team list, which assumes an event where teams are entered ahead of time. A practice field registers whoever turns up, and having to leave Match Play for the Free Practice page to set a key was the reason matches got set up on the wrong page. |
